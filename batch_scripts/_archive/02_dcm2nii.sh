...
echo -e "Starting ccp_dcm2nii_batch.sh"

# Source environment for dicom conversion
SETUP=$(get_value "SETUP_DCM2NII" "${CONFIG}")
echo ""
echo -e "SETUP SCRIPT\t:\t${SETUP}\n"
. ${SETUP}	# sources dcm2nii, ccp_dcm2nii, dicom_hinfo


# hard coded these. Need updating to handle gradient fieldmaps
DISCARD_WORDS="localizer bias aahscout scout nav norm dwi" # LIST OF DESCRIPTORS FOR IMAGES THAT SHOUDLN'T BE USED (lower case)
...
total_t1=$(grep -ic "T1" "${LIST}")
total_t2=$(grep -ic "T2" "${LIST}")
...
for ((i=1; i<=${total_spinecho}; ++i )) ; do
	...
	nii_folder="${OUT_DIR}/SpinEchoFieldMap${i}"
	mkdir -p ${nii_folder}
...
for ((i=1; i<=${total_t1}; ++i )) ; do
	...
	echo -e "\n\n\n"
	echo -e "Converting series ${IN_DIR}/${num_seriesT1W[$idx]}/DICOM to ${nii_folder}/${PREFIX}${nameT1W[$idx]}.nii.gz"
	...
	echo -e "\nNII exists?\t:\t${nii_exists}"
		...
		echo "NII.GZ filename\t:\t${filename}"	
            	...
            	cp "${filename}" "${nii_folder}/${PREFIX}${nameT1W[$idx]}${i}.nii.gz"
            	printf "%s %s\n" "${lineT1W[$idx]}" "${nii_folder}/${PREFIX}${nameT1W[$idx]}.nii.gz"  >> $TFILE
       	else
...
for ((i=1; i<=$total_t2; ++i )) ; do
	...
	echo -e "\n\n\n"
	echo -e "Converting series ${IN_DIR}/${num_seriesT2W[$idx]}/DICOM to ${nii_folder}/${PREFIX}${nameT2W[$idx]}.nii.gz"
	...
	if $nii_exists ;then #if there is a nii, gzip it (gaziop failed with the option '-g y' used in dcm2nii directly for very large files)
		...
		echo "NII.GZ filename\t:\t${filename}"
                ...
                cp "${filename}" "${nii_folder}/${PREFIX}${nameT2W[$idx]}${i}.nii.gz"
                printf "%s %s\n" "${lineT2W[$idx]}" "${nii_folder}/${PREFIX}${nameT2W[$idx]}.nii.gz"  >> $TFILE
...
## REST ##
##########
for ((i=1; i<=$total_rest; ++i )) ; do              # each i --> $ 4 rest series (RL normal and ref, LR normal and Ref) 
